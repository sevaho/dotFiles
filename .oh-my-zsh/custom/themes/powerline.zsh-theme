...
if [ "$POWERLINE_GIT_ADDED" = "" ]; then
  POWERLINE_GIT_ADDED="%K{red}%F{green}✚%F{red}"
fi

if [ "$POWERLINE_GIT_MODIFIED" = "" ]; then
  POWERLINE_GIT_MODIFIED="%K{red}%F{blue}✹%F{red}"
fi

if [ "$POWERLINE_GIT_DELETED" = "" ]; then
  POWERLINE_GIT_DELETED="%K{red}%F{black}✖%F{black}"
fi

if [ "$POWERLINE_GIT_UNTRACKED" = "" ]; then
  POWERLINE_GIT_UNTRACKED="%K{red}%F{yellow}✭%F{red}"
fi
...
# else
    if [ "$POWERLINE_SHOW_GIT_ON_RIGHT" = "" ]; then
        if [ "$POWERLINE_HIDE_GIT_PROMPT_STATUS" = "" ]; then
            ...
            POWERLINE_GIT_INFO_LEFT=" %F{blue}%K{red}"$'\ue0b0'"%F{red}%F{black}%K{red}"$'$(git_prompt_info)%F{red}'
        fi
        POWERLINE_GIT_INFO_RIGHT=""
    else
        POWERLINE_GIT_INFO_LEFT=""
        if [ "$POWERLINE_HIDE_GIT_PROMPT_STATUS" = "" ]; then
            POWERLINE_GIT_INFO_RIGHT="%F{red}"$'\ue0b2'"%F{black}%K{red}"$'$(git_prompt_info)$(git_prompt_status)'" %K{red}"
        else
            POWERLINE_GIT_INFO_RIGHT="%F{red}"$'\ue0b2'"%F{black}%K{red}"$'$(git_prompt_info)'" %K{red}"
        fi
    fi
# fi
